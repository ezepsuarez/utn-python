...
class Abm():
    ...
    def funcion_consultar(self, tree, var_mail, var_nombre, var_apellido, var_sexo, var_preferencia):
        ...
        if (var_nombre.get() == "" and var_mail.get() == "" and var_apellido.get() == "" and var_sexo.get() == "" and var_preferencia.get() == "" ):
            sql = (Usuario.select(Usuario.idusuario, Usuario.mail, Usuario.nombre, Usuario.apellido, Usuario.sexo, Categoria.id.alias('id'), Categoria.descripcion_categoria.alias('preferencia')).join(Categoria, on=(Usuario.preferencia == Categoria.id)).objects())
        else:
    ...
    def funcion_modificar(self, tree, master, mail, nombre, apellido, sexo, preferencia, boton_eliminar, boton_grabar, boton_consultar, boton_modificacion):
        ...
        if item == "" :
            ...
            boton_cancelar= Button(master, text=" Cancelar ", bg="#DC324C", command=lambda:boton.funcion_cancelar(tree, mail, nombre, apellido, sexo, preferencia, master, boton_modificacion, boton_grabar, boton_consultar, boton_eliminar), width=10)
...
class Pantalla():
    ...
    def funcion_cancelar(self, tree, master, mail, nombre, apellido, sexo, preferencia):
